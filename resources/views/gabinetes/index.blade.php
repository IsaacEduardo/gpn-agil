    ...
    <div class="container py-2">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0">
                ...
                <li class="breadcrumb-item active text-primary fw-bold" aria-current="page">Gabinetes</li>
            </ol>
        </nav>
    </div>
@endsection
...
@section('content')
    <div class="container pb-5">
        ...
        <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center mb-4 gap-3">
            <div>
                <h2 class="fw-bold text-dark mb-1"><i class="fas fa-building-user text-primary me-2"></i>Gabinetes</h2>
                <p class="text-muted mb-0 small">Gerencie os gabinetes da instituição.</p>
            </div>
            <a href="{{ route('gabinetes.create') }}" class="btn btn-primary shadow-sm fw-medium">
                <i class="fas fa-plus me-1"></i> Novo Gabinete
            </a>
        </div>
        ...
        <div class="card shadow-sm border-0 rounded-3 overflow-hidden">
                    ...
                    <tbody>
                        @forelse($gabinetes as $gabinete)
                            <tr>
                                <td class="ps-4">
                                    <div class="d-flex align-items-center">
                                        <div class="rounded-circle d-flex align-items-center justify-content-center me-3 fw-bold flex-shrink-0"
                                            ...
                                            {{ \Illuminate\Support\Str::upper(\Illuminate\Support\Str::substr($gabinete->sigla ?: $gabinete->nome, 0, 2)) }}
                                        </div>
                                        <div class="fw-bold text-dark">{{ $gabinete->nome }}</div>
                                    </div>
                                </td>
                                    ...
                                    @if ($gabinete->responsavel)
                                        <div class="d-flex align-items-center">
                                            <div class="rounded-circle bg-secondary-subtle text-secondary-emphasis d-flex align-items-center justify-content-center me-2 fw-semibold flex-shrink-0"
                                                ...
                                                {{ \Illuminate\Support\Str::upper(\Illuminate\Support\Str::substr($gabinete->responsavel->name, 0, 1)) }}
                                            </div>
                                            <span class="text-dark small">{{ $gabinete->responsavel->name }}</span>
                                        </div>
                                    @else
                                ...
                                <td>
                                    @if ($gabinete->superChefe)
                                        <div class="d-flex align-items-center">
                                            <div class="rounded-circle bg-info-subtle text-info-emphasis d-flex align-items-center justify-content-center me-2 fw-semibold flex-shrink-0"
                                                style="width: 30px; height: 30px; font-size: 0.75rem;">
                                                {{ \Illuminate\Support\Str::upper(\Illuminate\Support\Str::substr($gabinete->superChefe->name, 0, 1)) }}
                                            </div>
                                            <span class="text-dark small">{{ $gabinete->superChefe->name }}</span>
                                        </div>
                                    @else
                                        <span class="text-muted small">—</span>
                                    @endif
                                ...
                                <td class="text-end pe-4">
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end shadow-lg border-0">
                                            ...
                                            <li><a class="dropdown-item" href="{{ route('gabinetes.edit', $gabinete) }}"><i
                                                        class="fas fa-edit me-2 text-secondary w-20"></i>Editar</a></li>
                                            <li>
                                                <hr class="dropdown-divider">
                                            </li>
                                            <li>
                                                <form action="{{ route('gabinetes.destroy', $gabinete) }}" method="POST"
                            ...
                            <tr>
                                <td colspan="5" class="text-center py-5">
                                    <div class="d-flex flex-column align-items-center justify-content-center">
                                        ...
                                        <h5 class="fw-bold text-muted">Nenhum gabinete cadastrado</h5>
                                        <p class="text-muted small mb-3">Comece criando o primeiro gabinete.</p>
                                        <a href="{{ route('gabinetes.create') }}" class="btn btn-primary btn-sm">
                                            <i class="fas fa-plus me-1"></i> Novo Gabinete
                                        </a>
                                    </div>
                                </td>
            ...
            @if ($gabinetes instanceof \Illuminate\Contracts\Pagination\Paginator || $gabinetes instanceof \Illuminate\Pagination\LengthAwarePaginator)
                <div
                    ...
                    <small class="text-muted">Total de {{ $gabinetes->total() }} gabinete(s)</small>
                    {{ $gabinetes->onEachSide(1)->links('pagination::bootstrap-5') }}
                </div>
            @endif
        </div>
    </div>
