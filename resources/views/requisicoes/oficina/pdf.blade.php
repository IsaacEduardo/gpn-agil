...
<head>
    <meta charset="utf-8">
    <title>Ofício - Requisição de Oficina</title>
    <style>
        @page {
...
<body>
    ...
    <div class="content">
        <p>
            Para os devidos efeitos, remete-se a requisição para efeito de manutenção da viatura
            de marca <strong>{{ $viaturaInfo }}</strong> com a matrícula <strong>{{ $matricula }}</strong>,
            adstrita à Frota de Veiculos do Governo Provincial do Namibe.
        </p>
        @if ($quilometragem)
            <p>
                Quilometragem atual: <strong>{{ number_format($quilometragem, 0, ',', '.') }} km</strong>.
            </p>
        @endif
        <p>
            Descrição do serviço: {{ $descricaoProblema }}
            @if ($servicosSolicitados)
                <br>Serviços solicitados: {{ $servicosSolicitados }}
            @endif
        </p>
        <p>
