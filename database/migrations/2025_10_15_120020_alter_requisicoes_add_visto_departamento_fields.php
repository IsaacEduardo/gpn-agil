...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('requisicoes', function (Blueprint $table) {
            if (! Schema::hasColumn('requisicoes', 'visto_departamento_status')) {
                $table->string('visto_departamento_status', 20)
                    ->nullable()
                    ->after('status');
            }
            if (! Schema::hasColumn('requisicoes', 'visto_departamento_por')) {
            ...
            if (! Schema::hasColumn('requisicoes', 'visto_departamento_data')) {
                $table->dateTime('visto_departamento_data')
                    ->nullable()
                    ->after('visto_departamento_por');
            }
            if (! Schema::hasColumn('requisicoes', 'visto_departamento_observacao')) {
